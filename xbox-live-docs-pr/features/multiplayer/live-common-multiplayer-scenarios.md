...
Games cannot implement this inside their game today, but enable it's functionality out of game by advertising joinable activities through other means. That implmentation is done via other similar services explained in previous scenarios:
...
The Xbox Live SmartMatch service handles these restrictions automatically, and in ranked scenarios can opt out if that is not a requirement.
